...
export class RetrieveMonthlyReportUseCase {
  ...
  public async handle(user_id: string, year: number, month: number, options?: RetrieveAllOptionsInterface) {
    try {
      const weeksInYear = generateWeeksInYear(year);
      const weeksInMonth = weeksInYear.filter((weekData) => weekData.month === month);
      ...
      const report = await Promise.all(
        weeksInMonth.map(async (weekData) => {
